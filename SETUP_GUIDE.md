...
This guide walks you through setting up the lightweight prompt injection defense system.
...
## Step 2: Train Classifier in Google Colab

Training happens in Colab (~30-45 min), inference happens locally on CPU.

### 2.1 Open Training Notebook
...
3. Configure lightweight settings (distilbert-base-uncased, 66M params)
4. Load datasets (deepset, JailbreakBench)
5. Train model (~30-45 min on T4 GPU)
6. Evaluate metrics
7. Save final model to Google Drive
8. Download model zip file

### 2.3 Download and Extract Model

1. Download `prompt_injection_classifier.zip` from Colab
2. Extract the zip file locally
3. Move extracted folder to: `models/classifier/`
...
This runs comprehensive tests on the defense pipeline.

## Troubleshooting

### Model Not Found Error

If you get "Model not found at models/classifier":
1. Make sure you downloaded and extracted the model from Colab
2. Check that the folder structure is correct
3. Verify .env has `CLASSIFIER_MODEL_PATH=models/classifier`
...
3. **Paper Writing**: Use the evaluation metrics for your research paper
